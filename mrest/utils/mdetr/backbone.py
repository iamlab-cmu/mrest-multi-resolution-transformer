...
class BackboneBase(nn.Module):
    def __init__(self, backbone: nn.Module, train_backbone: bool, num_channels: int, return_interm_layers: bool, 
                 has_adapters: bool = False, train_adapters: bool = True,
                 train_layers: Optional[List[str]] = None):
        super().__init__()

        # MDETR by default does not train layer 1 or initial convs
        if train_layers is None:
            train_layers = ['layer2', 'layer3', 'layer4']
        
        def _is_param_in_layer(param_name: str):
            return all([l not in param_name for l in train_layers])

        for name, parameter in backbone.named_parameters():
            if "adapter" in name:
                assert has_adapters, 'Adapter found in model without adapters.'
                assert train_adapters, 'Should train adapters'
                parameter.requires_grad_(True)
            elif not train_backbone or _is_param_in_layer(name): 
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str, train_backbone: bool, return_interm_layers: bool, dilation: bool,
        ...
        norm_layer = FrozenBatchNorm2d if freeze_batchnorm else None
...
def build_backbone(args):
    ...
    elif args.get('backbone_adapters') and args.backbone_adapters.use:
