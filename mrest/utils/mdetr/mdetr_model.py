...
import mrest.utils.mdetr.util.dist as dist
from mrest.utils.mdetr.util import box_ops
from mrest.utils.mdetr.util.metrics import accuracy
from mrest.utils.mdetr.util.misc import NestedTensor, interpolate

from mrest.utils.mdetr.backbone import build_backbone
from mrest.utils.mdetr.matcher import build_matcher
from mrest.utils.mdetr.segmentation import dice_loss, sigmoid_focal_loss
from mrest.utils.mdetr.transformer import build_transformer
...
class MDETR(nn.Module):
    ...
    def __init__(
        ...
        use_all_encoded_img_tokens: bool = False,
    ):
        ...
        # Control readout
        self.use_control_readout_emb = use_control_readout_emb
        self.use_all_encoded_img_tokens = use_all_encoded_img_tokens

    def forward_images(self, samples: torch.Tensor, captions, proprio_z: Optional[torch.Tensor] = None):
        bs, _, img_h, img_w = samples.size()
        # TODO: mask allows us to get binary values for masked regions. Are these useful?
        # mask = torch.zeros((bs, img_h, img_w), dtype=torch.bool, device=samples.device)
        # samples = NestedTensor.from_tensor_list(samples)
        
        memory_cache = self._forward_images(samples, captions, encode_and_save=True, proprio_z=proprio_z)
        
        if self.use_control_readout_emb:
            return memory_cache['ctrl_pooled_op'], memory_cache["text_embedding"]
        elif self.use_all_encoded_img_tokens:
            return memory_cache
        else:
            return memory_cache
            # outputs = self._forward_images(samples, captions, encode_and_save=False, memory_cache=memory_cache)
            # return outputs, memory_cache["text_embedding"]
    
    def forward_unimodal_input(self, samples: torch.Tensor, captions: List[str],
                               proprio_z: Optional[torch.Tensor] = None):
        ...
        memory_cache = self.transformer.prepare_mdetr_input(
            self.input_proj(src),
            mask,
            query_embed,
            pos_embed=pos[-1],
    ...
    def _forward_images(self, samples: torch.Tensor, captions, encode_and_save: bool = False, memory_cache = None,
                        proprio_z: Optional[torch.Tensor] = None):
        if not isinstance(samples, NestedTensor):
            samples = NestedTensor.from_tensor_list(samples)
        ...
        if encode_and_save:
            ...
            features, pos = self.backbone(samples)
            src, mask = features[-1].decompose() # src = [64, 2048, 7, 7], mask = [64, 7, 7]
            query_embed = self.query_embed.weight
            if self.qa_dataset is not None:
                query_embed = torch.cat([query_embed, self.qa_embed.weight], 0)
            # Note: Shape of src = [bs, 2048, 7, 7]; Shape of self.input_proj(src) = [bs, 256, 7, 7] where 256 is the number of channels
